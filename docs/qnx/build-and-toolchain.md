...
- For QNX® SDP 8.0, install "Notification FD Interfaces" from QNX Software
  Center so `libeventfd` is available.  Builds that link eventfd users require
  this package.
- Use the bundled/in-tree fontconfig path generated by the QNX bootstrap
  (`use_bundled_fontconfig = true`).  The QNX SDP 8 system fontconfig is too
  old for Chromium 147's current fontconfig symbols.
...
# Unsupported or deferred integrations.
...
# Linux sandbox gates are disabled until a QNX-native sandbox exists.
...
# Font matching uses Chromium's in-tree fontconfig on QNX.
use_bundled_fontconfig = true
```

The release bootstrap also disables unsupported desktop/media integrations
(ALSA/PulseAudio, X11/GTK/DBus, VA-API, PipeWire, libpci, etc.) and lowers
symbol levels so non-component QNX builds remain linkable in the current
environment.
...
QNX uses Chromium's in-tree fontconfig instead of the QNX SDP 8 system
fontconfig.  The SDP library lacks symbols required by Chromium 147 (for
example `FcWeightFromOpenType`), while CEF API tests need real font-family
matching for text layout and hit testing.

Current QNX font behavior is therefore:

- `use_bundled_fontconfig = true` in generated `args.gn`
- Skia's fontconfig-backed `SkFontMgr` is wired for QNX
- Linux font-cache and font-fallback sources are reused where they are
  POSIX/fontconfig based rather than Linux-kernel specific
- older QNX-only hard-coded font stubs are retired in favor of the real
  fontconfig path
...
|---|---|
| existing Chromium file | `cef/patch/patches/qnx/chromium/*.patch` plus a `cef/patch/patch.cfg` entry |
| existing submodule file | `cef/patch/patches/qnx/*.patch` plus a `patch.cfg` entry when required |
...
- register every bootstrap-applied patch in `cef/patch/patch.cfg`; orphan patch files under `patches/qnx/chromium/` are not durable fixes
