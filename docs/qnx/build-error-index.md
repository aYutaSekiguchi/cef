...
rg -n "ceftests|per-test|TMPDIR|ProcessSingleton|slay|AudioManager|CloudProfileReportingServiceFactory|NotificationPlatformBridge|BrowserNativeWidget|DeveloperToolsAvailabilityAllowlist|devtools.availability_allowlist|AxViewportCollapseTest|BrowserSettingsTest|gtest_list_tests|SkFree|SkTDArray|sk_realloc_throw|UncheckedRealloc|SkMemory_new_handler|cross-DSO|PartitionAlloc|MaybeHandleSystemDeallocation|__free|offsets_to_metadata|mcmodel=medium|SIGSEGV|lea.*edx|FromFirstSuperPage" docs/qnx/history/build-errors/test
