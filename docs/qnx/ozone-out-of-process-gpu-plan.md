...
- `out/qnx_release/cefsimple` (3.3 MiB) — clean-bootstrap rebuild succeeded 2026-07-10 (`ninja_qnx.sh cefsimple -k 20`, 78,689/78,689 actions, exit 0); `locales/` is now yuta-owned mode 0775. Default CEF Views remains a separate startup blocker (exit 139, or exit 1 with Mesa preloaded) after `QnxGpuService::Initialize` **before** `OzonePlatformQnx::CreatePlatformWindow`. Native CEF (`--use-native --url=about:blank`) has a verified loader fix: without preload its `libcef.so`-selected unversioned ANGLE `libEGL.so` reports absent Mesa DMAbuf export capability; with `LD_PRELOAD=/usr/lib/libEGL.so.1`, it selects QNX Mesa 1.5 and reaches `SubmitFrame: FINAL accepted=true display_ok=true; eglSwapBuffers reached`. The durable candidate is replacing QNX's `//third_party/angle:libEGL` CEF dependency with system `EGL` linkage; no behavioral source fix is applied yet.
