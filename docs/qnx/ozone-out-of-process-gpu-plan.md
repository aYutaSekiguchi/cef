...
Status: **build progressed (2026-07-09); headless run + screenshot pending.** Phase 5 + Phase 6 produced a content_shell binary whose out-of-process GPU + DMAbuf pipeline painted a 800x600 sky-blue rectangle through QEMU virgl (user-verified visually). Phase 7 build-time evidence recorded in this session:

- `out/qnx_release/content_shell` (883 MiB) — already built (Phase 5/6), produced 13-step QNX_OZONE_GPU_TRACE through `eglSwapBuffers reached` in the no-v=1 path; user-verified sky-blue paint via QEMU GTK window.
- `out/qnx_release/base_unittests` (199 MiB) — built in this session (`ninja -C out/qnx_release -j10 base_unittests`, 505/505 actions). Valid QNX ELF; gtest symbols present.
- `out/qnx_release/url_unittests` (52 MiB) — built in this session (33/33 actions). Valid QNX ELF.

Phase 7 needs to:

1. Build `cefsimple` and `cefclient` with `--ozone-platform=qnx` and verify the CEF runtime can spawn an embedded window.
2. Build a richer test-page rendering path beyond the solid-color DMAbuf test (e.g., render a real HTML page in Chromium content_shell and capture a screenshot).
3. Capture an automated screenshot (rather than relying on the human to view the QEMU GTK window).
4. Re-run `base_unittests` and one Chromium telemetry test on QNX to prove no regression.

Acceptance evidence:

- Build command/output summary (content_shell + cefsimple).
- QEMU command (--virgl).
- Screenshot path (PNG file generated by `PrintToFileRequest` or `xvfb-style --screenshot=path` Chromium switch).
- Headless regression command/output summary (`./out/qnx_release/ninja_qnx.sh base_unittests`, one Chromium test).

- [ ] Build required visual targets. (content_shell already built; `cefsimple` target needs to be added to the QNX GN args + a known-good cefsimple GN args patch.)
- [ ] Run QEMU virgl smoke with out-of-process GPU. (Done in Phase 5 — `content_shell --ozone-qnx-gpu-trace about:blank` reaches eglSwapBuffers; deterministic since Phase 6 ACK fix.)
- [ ] Run `cefsimple --ozone-platform=qnx` without `--in-process-gpu`. (Pending — depends on the build target above. Once cefsimple is built, the smoke command is `qnx_run.sh --virgl -- ./cefsimple --ozone-platform=qnx --use-gl=egl --no-sandbox about:blank`.)
- [ ] Capture screenshot. (Phase 5 captured sky-blue paint manually via the QEMU GTK window. Phase 7 needs an automated PNG capture, ideally via Chromium's `PrintToFileRequest` Mojo path or via the QEMU screendump knob `screendump /tmp/screen.ppm`. The QEMU PPM dump from the host's `/tmp/qemu*.pmm` is a known path: `kill -SIGUSR1 $(pidof qemu-system-x86_64)` writes the host-side QEMU display buffer to a `.ppm` file.)
- [x] Build base_unittests, url_unittests for QNX. (base_unittests 199 MiB + url_unittests 52 MiB built 2026-07-09 against the post-Phase 6 binary. Other suites (e.g. net_unittests) hit a separate QNX-only patch gap, `net::DelayedCookieMonster` is abstract under the QNX libc++ macros; that's a Phase 8+ gap, not a Phase 7 blocker.)  
- [ ] Run base_unittests and url_unittests on QEMU. (Pending — host QEMU env wedged after many smoke cycles. User-side: `./out/qnx_release/ninja_qnx.sh base_unittests` is no-op (built), so the work is just running on QEMU.)
